...
class jpsw_expansion_policy : public expansion_policy
{
    ...
    private:
        void
        reach(uint32_t id, warthog::jps::direction direction, double g, warthog::problem_instance* pi);

        void
        prospect(uint32_t id, double g, bool ortho, warthog::problem_instance* pi);

        int
        calculate_successors(uint32_t source);
        int
        nbhood_successors(uint32_t to, warthog::jps::direction going);

        inline double
        diagonal_cost(uint32_t nw_corner)
        {
        ...
        void
        calculate_jump(uint32_t start, int delta, uint32_t version, F cost);

        void
        jump_west(uint32_t from, nbhood_labels nb, double g, double cost, warthog::problem_instance* pi);
        void
        jump_east(uint32_t from, nbhood_labels nb, double g, double cost, warthog::problem_instance* pi);
        void
        jump_north(uint32_t from, nbhood_labels nb, double g, double cost, warthog::problem_instance* pi);
        void
        jump_south(uint32_t from, nbhood_labels nb, double g, double cost, warthog::problem_instance* pi);

        void
        jump_nw(nbhood_labels nb, double g, int successor_set, warthog::problem_instance* pi);
        void
        jump_ne(nbhood_labels nb, double g, int successor_set, warthog::problem_instance* pi);
        void
        jump_sw(nbhood_labels nb, double g, int successor_set, warthog::problem_instance* pi);
        void
        jump_se(nbhood_labels nb, double g, int successor_set, warthog::problem_instance* pi);
