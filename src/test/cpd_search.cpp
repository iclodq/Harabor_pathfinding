...
SCENARIO("Test reverse CPD A* on a square matrix", "[reverse][square][astar]")
{
    ...
    warthog::graph::gridmap_to_xy_graph(&d, &g, false);
    warthog::simple_graph_expansion_policy expander(&g);
    warthog::cpd::reverse_oracle oracle(&g);
    ...
    GIVEN("A reverse CPD")
    {
        ...
        {
            warthog::cpd_extractions<warthog::cpd::reverse_oracle> cpd(
                &g, &oracle);
            warthog::solution sol;
        ...
        THEN("Search should not expand nodes without congestion")
        {
            warthog::solution sol;
            warthog::pqueue_min open;
            warthog::cpd_heuristic_base<warthog::cpd::reverse_oracle> h(&oracle);
            warthog::cpd_search<
                warthog::cpd_heuristic_base<warthog::cpd::reverse_oracle>,
                warthog::simple_graph_expansion_policy>
                    astar(&h, &expander, &open);
